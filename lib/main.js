...
var match_css_tags = /<link[^>]+href=[^>]+(\.css)[^>]+>/ig;
var match_end_of_head = /(<\/head.*)/i;
...
pagespeed.css_to_head = function ( output ) {
  var tags = [], tag;
  while ( tag = match_css_tags.exec( output )) {
    tags.push( tag[0] );
  }
  output = output.replace( match_css_tags, '');
  var self = this;
  output = output.replace( match_end_of_head, function( str, end_of_head ) {
    return tags.join('') + end_of_head;
  } );
  return output;
}
